...
const generateResponse = async (req, res) => {
  ...
  const { userPrompt, tweakingParameters, shortenedInputPrompt } = req.body;
  ...
  if (user.subscription === "free" && user.uses <= 1) {
    return res
      .status(402)
      .json({ message: "Out of uses. Please upgrade your plan." });
  }
  ...
  try {
    ...
    const result = await threadModel.findOne({
      ...
      userId: userId,
      shortenedInputPrompt: shortenedInputPrompt,
    });
    if (!result) {
